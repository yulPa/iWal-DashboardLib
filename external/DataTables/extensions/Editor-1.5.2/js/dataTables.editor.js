	...
	_multiValueCheck: function () {
		...
		if ( ids.length > 1 ) {
			this.dom.multiReturn.css( {
				display: different && ! this.s.multiValue ? 'block' : 'none'
			} );
		}
...
$.extend( Editor.DateTime.prototype, {
	...
	val: function ( set, write ) {
		...
		if ( set instanceof Date ) {
			...
			if ( this.c.format === 'YYYY-MM-DD' ) {
				...
				var m = window.moment( set, this.c.format, this.c.momentLocale, this.c.momentStrict );
		...
		this.dom.container
			.on( 'change', 'select', function () {
				...
				if ( select.hasClass(classPrefix+'-month') ) {
					// Month select
					that.s.display.setMonth( val );
					that._setTitle();
					that._setCalander();
				}
				else if ( select.hasClass(classPrefix+'-year') ) {
					// Year select
					that.s.display.setFullYear( val );
					that._setTitle();
					that._setCalander();
				}
			...
			.on( 'click', function (e) {
				...
				if ( nodeName === 'button' ) {
					...
					if ( parent.hasClass(classPrefix+'-iconLeft') ) {
						// Previous month
						that.s.display.setMonth( that.s.display.getMonth()-1 );
						that._setTitle();
						that._setCalander();
	...
	_htmlMonth: function ( year, month ) {
		...
		for ( var i=0, r=0 ; i<cells ; i++ ) {
			...
			if ( $.isArray( disableDays ) && $.inArray( day.getDay(), disableDays ) !== -1 ) {
				disabled = true;
			}
	...
	_setCalander: function () {
		this.dom.calendar
			...
			.append( this._htmlMonth(
				this.s.display.getFullYear(),
				this.s.display.getMonth()
			) );
	},
	 ...
	 */
	_setTitle: function () {
		this._optionSet( 'month', this.s.display.getMonth() );
	...
	_setTime: function () {
		...
		this._optionSet( 'minutes', d ? d.getMinutes() : 0 );
	...
	_show: function () {
		...
		// Hide if clicking outside of the widget
	...
	_writeOutput: function () {
		var date = this.s.d;
		var out = this.c.format === 'YYYY-MM-DD' ?
			date.getUTCFullYear() +'-'+
